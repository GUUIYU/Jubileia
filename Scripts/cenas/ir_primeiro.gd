	...
	if Input.is_action_just_pressed("proximo") and !escolhendo:
		...
		if anm < total:
			...
			if final == "cima":
				...
				if Globais.informacao:
					get_tree().change_scene_to_file("res://Cenas/caminhos/metro/paris/explorar_cafe.tscn")
				else:
					get_tree().change_scene_to_file("res://Cenas/caminhos/metro/perdida.tscn")
